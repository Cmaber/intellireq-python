...
def get_bert_predictions(sentence, interested_pos):
    ...
    results = {}
    for word in doc:
        if word.pos_ in interested_pos and word.text != 'system':
            try:
...
def similarCheck(requirement1, requirement2, glove_embeddings):
...
if __name__ == '__main__':
    ...
    while True:
        print("\nEnter two sentences to compare their similarity, or type 'exit' to quit.")

        requirement1 = input("Enter the first requirement: ")
        if requirement1.lower() == 'exit':
            break

        requirement2 = input("Enter the second requirement: ")
        if requirement2.lower() == 'exit':
            break

        similarCheck(requirement1, requirement2, glove_embeddings)
